---
title: Provenance and Decision Records
layer: research
status: stable
version: 0.4.0
updated: 2026-09-15
---

# 05 · Provenance and Decision Records

本目录保存来源身份映射与 Project Aletheia 自身的决定确认记录。它解决两个不同问题：某项材料来自哪里，以及项目发起者具体确认了什么。

- [`SOURCE_REGISTRY.md`](SOURCE_REGISTRY.md)：为来源提取、导入记录和本轮项目对话分配稳定、不携带语义的 Source ID。
- [`PROJECT_DECISION_CONFIRMATIONS.md`](PROJECT_DECISION_CONFIRMATIONS.md)：记录项目内部方向决定的确认主体、确认范围及所依赖的 Source ID。
- [`PROJECT_INITIATOR_CONFIRMATION_v0.2.4.md`](PROJECT_INITIATOR_CONFIRMATION_v0.2.4.md)：记录 G-01—G-20、C-01—C-14 按建议状态全部确认的逐项边界。
- [`PROJECT_INITIATOR_CONFIRMATION_v0.2.5.md`](PROJECT_INITIATOR_CONFIRMATION_v0.2.5.md)：记录十环节矩阵、三级触发及现行术语澄清的确认边界。
- [`PROJECT_INITIATOR_CONFIRMATION_v0.2.6.md`](PROJECT_INITIATOR_CONFIRMATION_v0.2.6.md)：记录责任会计三分结构正式取代 `C-13` 的确认边界。
- [`PROJECT_INITIATOR_CONFIRMATION_v0.2.7.md`](PROJECT_INITIATOR_CONFIRMATION_v0.2.7.md)：记录年度责任积分、领域累计贡献、质量评价、专业参考摘要及两个外部文件的整合边界。
- [`PROJECT_INITIATOR_CONFIRMATION_v0.2.8.md`](PROJECT_INITIATOR_CONFIRMATION_v0.2.8.md)：记录验证状态第二维度与权力—责任—决策交叉审计的确认边界。
- [`PROJECT_INITIATOR_CONFIRMATION_v0.2.9.md`](PROJECT_INITIATOR_CONFIRMATION_v0.2.9.md)：记录边界案例、探索稿反馈登记和原典核查优先级的确认边界。
- [`PROJECT_INITIATOR_CONFIRMATION_v0.3.0.md`](PROJECT_INITIATOR_CONFIRMATION_v0.3.0.md)：记录 `v0.3.0` 公共研究快照、反馈后三步稿与发布边界；不把草案公开等同于作者定稿或现实验证。
- [`PROJECT_INITIATOR_CONFIRMATION_v0.4.0.md`](PROJECT_INITIATOR_CONFIRMATION_v0.4.0.md)：记录 `v0.4.0` 公共研究快照、哲学理论版本名称与发布边界；不把理论桥梁公开等同于外部验证或现实授权。

`v0.1.6` 的 `PCR-5E7A91` 记录有限互惠公共责任方向及其边界，并明确其只部分取代 `D-015` 的开放结论，不取消当时三层不得自动跳转的边界。`PCR-A18D8A` 与 `D-028` 又补入公共需求归属责任；现行术语展示为五个判断：公共必要性、公共需求归属责任、公共组织与维护责任、有限互惠公共责任与具体公共责任任务义务。

`v0.2.2` 的 `PCR-C35F82` 和 [`PROJECT_INITIATOR_CONFIRMATION_v0.2.2.md`](PROJECT_INITIATOR_CONFIRMATION_v0.2.2.md) 记录职业开放、司法四层、一般人身安全及生育婚姻亲职分离 37 项方向全部接受；原 RTF 参数和现实机制不因此自动确认。

`v0.2.3` 的 `PCR-C49CCC` 记录个人需求责任、共同脆弱性、非等级互助、跨时间传递、基本照护不作功劳结算和不制造感恩服从的确认范围；它不建立个人归罪、互助积分、感恩考核或具体照护参数。

`v0.2.4` 的 `PCR-7B4D2E` 记录公共权力失效纠错与公共合作激励方向；删除债务劳动、统一责任积分和综合可靠度，具体调查、基金、追偿、资产和补偿机制继续开放。

`v0.2.5` 的 `PCR-9C2F61` 记录十环节权力集中控制矩阵方向；环节数量只触发审查和纠偏，不建立权力总分、自动腐败推定或责任自动授权。

`v0.2.6` 的 `PCR-6D2A8F` 记录周期性基础责任账户、公共贡献履历和领域限定的专业可靠性档案三分结构。`D-024` 取代 v0.2.4 `C-13` 的全面删除，但不恢复终身总分、人格评分、基本保障绑定或自动派工。

`v0.2.7` 的 `PCR-A2D7F9` 将三分记录细化为四层功能，接受年度清零、任务当量结构、领域累计贡献、质量评价和专业参考摘要；具体参数和现实效力继续开放。

`v0.2.8` 的 `PCR-5E32B8` 将文档成熟度与验证进度分开，并确认三模型交叉审计只负责发现和路由接口问题，不取代原模型或冒充现实验证。

`v0.2.9` 的 `PCR-9F3A72` 将首批历史检验收窄为两个具体案例，建立外部反馈登记并按对第一原则的实际支撑关系排序原典核查；待发布稿和不存在的反馈不得预填。

`v0.3.0` 的 `PCR-B60C2F` 与 `D-029` 记录七条公众命题和五篇思想书中心问题的作者确认，并让有限公共责任个案继续保持候选状态。该记录只把相关文稿推进到部分作者审阅，不表示全文定稿、事实核查、现实验证或实施授权完成。

`v0.3.0` 的 `PCR-4D7A31` 与 `D-030` 冻结第一原则 v1.0 的十一公理与禁区规范语义；`PCR-E9C2B5` 与 `D-031` 记录司法解释、四份人生周期走查、缺席后果候选机制、40 岁转行外部验证和生育禁止现实试验的范围；`PCR-C47D91` 与 `D-032` 只确认公开研究快照及反馈后三步稿的发布边界。三项确认都不把草案、事实、参数、验证结果或实施权限升级为现实结论。

`Unreleased` 的 `PCR-57C2A8` 与 `D-033` 将 40 岁转行首次外部验证收窄为上海市普通企业职工主动离职后转向养老护理员。它只固定研究范围，不确认个人资格、经办结果、复核分组、录用、补贴、现实安全或方案效果；七类复核视角仍是草案中的安全性设计，方案继续保持 `not-ready`，邀请尚未发出。

`PCR-3F8A6D` 与 `D-034` 进一步确认六项首轮比较设置：上海社保缴费地、普通主动辞职、零相关经验资格、备案养老机构直接用工主线、五项敏感性变量，以及不预设就业困难人员认定。确认对象是研究假设，不是现实人物事实；实践预核、参与保护和外部验证仍未完成。

`PCR-6E4A91B2` 与 `D-035` 确认哲学理论五项桥梁进入框架层：四类命题层级、弱者选择权、创新公共回流、劳动主体承认与冲突不可逆伤害边界。它不修改第一原则 v1.0，也不证明经验假设或机制效果。

`PCR-4E0A1F` 与 `D-036` 确认 `v0.4.0` 公共研究快照发布范围，版本名称为“Aletheia 哲学理论 v0.4.0：人的主体性、公共责任与制度边界”。发布只冻结公开研究快照，不提升草案、事实核查、验证状态或现实授权。

这里的记录只证明 Project Aletheia 的思想归属和项目内部决定链，**不是**现实共同体、受影响者或公众对任何制度的授权。项目内部确认不能替代公共决策所需的参与、异议、代表、复核和撤回程序。

## 记录顺序

```text
来源进入项目并取得 Source ID
→ 起草命题或决定
→ 记录确认主体、来源与范围
→ DECISION_LOG 标记 Accepted
→ 同步术语、理论、文档状态和 Unreleased 变更记录
```

Source ID 与决定确认记录必须在语义冻结前形成；压缩包、校验和及最终发布版本在结构冻结后形成。
