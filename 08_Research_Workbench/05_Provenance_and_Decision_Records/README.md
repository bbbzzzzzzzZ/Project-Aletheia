...
本目录保存来源身份映射与 Project Aletheia 自身的决定确认记录。它解决两个不同问题：某项材料来自哪里，以及项目发起者具体确认了什么。

- [`SOURCE_REGISTRY.md`](SOURCE_REGISTRY.md)：为来源提取、导入记录和本轮项目对话分配稳定、不携带语义的 Source ID。
- [`PROJECT_DECISION_CONFIRMATIONS.md`](PROJECT_DECISION_CONFIRMATIONS.md)：记录项目内部方向决定的确认主体、确认范围及所依赖的 Source ID。
- [`PROJECT_INITIATOR_CONFIRMATION_v0.2.4.md`](PROJECT_INITIATOR_CONFIRMATION_v0.2.4.md)：记录 G-01—G-20、C-01—C-14 按建议状态全部确认的逐项边界。
...
`v0.2.3` 的 `PCR-C49CCC` 记录个人需求责任、共同脆弱性、非等级互助、跨时间传递、基本照护不作功劳结算和不制造感恩服从的确认范围；它不建立个人归罪、互助积分、感恩考核或具体照护参数。

`v0.2.4` 的 `PCR-7B4D2E` 记录公共权力失效纠错与公共合作激励方向；删除债务劳动、统一责任积分和综合可靠度，具体调查、基金、追偿、资产和补偿机制继续开放。

`v0.2.5` 的 `PCR-9C2F61` 记录十环节权力集中控制矩阵方向；环节数量只触发审查和纠偏，不建立权力总分、自动腐败推定或责任自动授权。
...
→ 起草命题或决定
→ 记录确认主体、来源与范围
→ DECISION_LOG 标记 Accepted
→ 同步术语、理论、文档状态和 Unreleased 变更记录
```

Source ID 与决定确认记录必须在语义冻结前形成；压缩包、校验和及最终发布版本在结构冻结后形成。
